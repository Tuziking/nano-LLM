...
class LLM(PreTrainedModel):
    ...
    def __init__(self, config):
        ...
        for pn, p in self.named_parameters():
            if pn.endswith("up_proj.weight") or pn.endswith("o_proj.weight"):
    ...
    def forward(self, input_ids, labels=None, use_kv_cache=False):
        ...
        if labels is not None:
            ...
            self.loss = F.cross_entropy(logits.view(-1, logits.size(-1)), labels.view(-1), ignore_index=pad_id)
        else:
            logits = self.output(hidden_states[:, [-1], :])
            self.loss = None

        return CausalLMOutputWithPast(loss=self.loss, logits=logits)

    
    @torch.inference_mode
    def generate(
        self, inputs, eos, max_new_tokens,
        temperature=0.7, top_k=None, stream=True,
        ...
        while input_ids.shape[1] < max_new_tokens - 1:
            ...
            if no_repeat_ngram_size > 0 and input_ids.shape[1] >= no_repeat_ngram_size - 1:
                ...
                for i in range(len(generated) - no_repeat_ngram_size + 1):
                    prefix = tuple(generated[i:i + no_repeat_ngram_size - 1])
                    next_token = generated[i + no_repeat_ngram_size - 1]
...
if __name__ == '__main__':   
    ...
    print(f"参数总数: {total:,}, 可训练参数: {trainable:,}")


    data_collator = DefaultDataCollator()
    wandb.init(project="nanoLLM", name="model1_4epoch_2")
    tokenizer = AutoTokenizer.from_pretrained("./tokenizer", use_fast=True)
    args = TrainingArguments(output_dir='checkpoints/model1_4epoch_2', 
                            num_train_epochs=1, 
                            do_train=True, 
    ...
    trainer.save_model('./saves/model1_4epoch_2')
    tokenizer.save_pretrained('./saves/model1_4epoch_2/tokenizer')  # 保存 tokenizer